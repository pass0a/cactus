...
// end of this file. dr_wav - v0.9.0 - 2018-12-16
//
...
DEPRECATED APIS
===============
Version 0.9.0 deprecated the per-sample reading and seeking APIs and replaced
them with versions that work on the resolution of a PCM frame instead. For
example, given a stereo WAV file, previously you would pass 2 to
drwav_read_f32() to read one PCM frame, whereas now you would pass in 1 to
drwav_read_pcm_frames_f32(). The old APIs would return the number of samples
read, whereas now it will return the number of PCM frames. Below is a list of
APIs that have been deprecated and their replacements.
...
Rationale:
    1) Most programs will want to read in multiples of the channel count which
demands a per-frame reading API. Per-sample reading just adds complexity and
maintenance costs for no practical benefit. 2) This is consistent with my other
decoders - dr_flac and dr_mp3.

These APIs will be removed completely in version 0.10.0. You can continue to use
drwav_read_raw() if you need per-sample reading.
*/
...
//     }
//
//     drwav_int32* pDecodedInterleavedSamples = malloc(wav.totalPCMFrameCount *
//     wav.channels * sizeof(drwav_int32)); size_t
//     numberOfSamplesActuallyDecoded = drwav_read_pcm_frames_s32(&wav,
...
//     float* pSampleData =
//     drwav_open_file_and_read_pcm_frames_f32("my_song.wav", &channels,
//     &sampleRate, &totalPCMFrameCount); if (pSampleData == NULL) {
...
// the audio data in its internal format (see notes below for supported
// formats):
//
//     size_t samplesRead = drwav_read_pcm_frames(&wav, wav.totalPCMFrameCount,
...
// drwav_open_file_write(), etc. Use drwav_write_pcm_frames() to write samples,
...
//     ...
//
//     drwav_uint64 samplesWritten = drwav_write_pcm_frames(pWav, frameCount,
//     pSamples);
//
//
...
// drwav_read_pcm_frames_f32(), drwav_read_pcm_frames_s32()
//   and drwav_read_pcm_frames_s16() to read and convert audio data to 32-bit
//   floating point, signed 32-bit integer and signed 16-bit integer samples
//   respectively. Tested and supported internal formats include the following:
//   - Unsigned 8-bit PCM
//   - Signed 12-bit PCM
//   - Signed 16-bit PCM
//   - Signed 24-bit PCM
...
// parameter of drwav_init_ex(), drwav_open_ex() and family. onRead [in] A
// pointer to the function to call when reading. onSeek            [in] A
// pointer to the function to call when seeking. pReadSeekUserData [in] The user
// data that was passed to the pReadSeekUserData parameter of drwav_init_ex(),
// drwav_open_ex() and family. pChunkHeader      [in] A pointer to an object
// containing basic header information about the chunk. Use this to identify the
// chunk.
//
...
typedef struct {
  ...
  // non-sequential writes and when the drwav object is opened in read mode.
  // Used for validation.
  ...
  struct {
    ...
    drwav_int32 prevSamples[2][2]; // The previous 2 samples for each channel (2
                                   // channels at most).
  } msadpcm;
  ...
  drwav_uint64
      totalSampleCount; // <-- DEPRECATED. Will be removed in a future version.
} drwav;

// Initializes a pre-allocated drwav object.
//
// pWav                         [out]          A pointer to the drwav object
// being initialized. onRead                       [in]           The function
// to call when data needs to be read from the client. onSeek [in]           The
// function to call when the read position of the client data needs to move.
// onChunk                      [in, optional] The function to call when a chunk
// is enumerated at initialized time. pUserData, pReadSeekUserData [in,
// optional] A pointer to application defined data that will be passed to onRead
// and onSeek. pChunkUserData               [in, optional] A pointer to
// application defined data that will be passed to onChunk. flags [in, optional]
...
//   DRWAV_SEQUENTIAL: Never perform a backwards seek while loading. This
//   disables the chunk callback and will cause this function
//                     to return as soon as the data chunk is found. Any chunks
//                     after the data chunk will be ignored.
//
// drwav_init() is equivalent to "drwav_init_ex(pWav, onRead, onSeek, NULL,
// pUserData, NULL, 0);".
//
// The onChunk callback is not called for the WAVE or FMT chunks. The contents
// of the FMT chunk can be read from pWav->fmt after the function returns.
//
// See also: drwav_init_file(), drwav_init_memory(), drwav_uninit()
drwav_bool32 drwav_init(drwav *pWav, drwav_read_proc onRead,
                        drwav_seek_proc onSeek, void *pUserData);
drwav_bool32 drwav_init_ex(drwav *pWav, drwav_read_proc onRead,
                           drwav_seek_proc onSeek, drwav_chunk_proc onChunk,
                           void *pReadSeekUserData, void *pChunkUserData,
                           drwav_uint32 flags);
...
// or from a block of memory respectively.
//
// If the total sample count is known, you can use
// drwav_init_write_sequential(). This avoids the need for dr_wav to perform a
// post-processing step for storing the total sample count and the size of the
// data chunk which requires a backwards seek.
//
// If you want dr_wav to manage the memory allocation for you, consider using
// drwav_open() instead. This will allocate a drwav object on the heap and
// return a pointer to it.
...
//
// You can also use drwav_open_file() and drwav_open_memory() to open the stream
// from a file or from a block of memory respectively.
//
...
drwav *drwav_open(drwav_read_proc onRead, drwav_seek_proc onSeek,
                  void *pUserData);
drwav *drwav_open_ex(drwav_read_proc onRead, drwav_seek_proc onSeek,
...
// Close the loader with drwav_close().
//
// You can also use drwav_open_file_write() and drwav_open_memory_write() to
...
//
// If the return value is less than <framesToRead> it means the end of the file
...
// drwav_read_raw() or drwav_read_pcm_frames_s16/s32/f32/etc().
drwav_uint64 drwav_read_pcm_frames(drwav *pWav, drwav_uint64 framesToRead,
                                   void *pBufferOut);
...
// If the return value is less than <framesToRead> it means the end of the file
// has been reached.
drwav_uint64 drwav_read_pcm_frames_s16(drwav *pWav, drwav_uint64 framesToRead,
...
// If the return value is less than <framesToRead> it means the end of the file
// has been reached.
drwav_uint64 drwav_read_pcm_frames_f32(drwav *pWav, drwav_uint64 framesToRead,
                                       float *pBufferOut);
...
// If the return value is less than <framesToRead> it means the end of the file
// has been reached.
drwav_uint64 drwav_read_pcm_frames_s32(drwav *pWav, drwav_uint64 framesToRead,
...
drwav_bool32 drwav_init_file_ex(drwav *pWav, const char *filename,
...
drwav *drwav_open_file_ex(const char *filename, drwav_chunk_proc onChunk,
                          void *pChunkUserData, drwav_uint32 flags);
...
drwav_bool32 drwav_init_memory_ex(drwav *pWav, const void *data,
                                  size_t dataSize, drwav_chunk_proc onChunk,
                                  void *pChunkUserData, drwav_uint32 flags);
...
drwav *drwav_open_memory_ex(const void *data, size_t dataSize,
                            drwav_chunk_proc onChunk, void *pChunkUserData,
                            drwav_uint32 flags);
...
float *drwav_open_file_and_read_pcm_frames_f32(const char *filename,
                                               ...
                                               unsigned int *sampleRate,
                                               drwav_uint64 *totalFrameCount);
drwav_int32 *drwav_open_file_and_read_pcm_frames_s32(
    const char *filename, unsigned int *channels, unsigned int *sampleRate,
    drwav_uint64 *totalFrameCount);
#endif

// Opens and decodes a wav file from a block of memory in a single operation.
drwav_int16 *drwav_open_memory_and_read_pcm_frames_s16(
...
drwav_int32 *drwav_open_and_read_s32(drwav_read_proc onRead,
                                     ...
                                     drwav_uint64 *totalSampleCount);
#ifndef DR_WAV_NO_STDIO
drwav_int16 *drwav_open_memory_and_read_s16(const void *data, size_t dataSize,
                                            unsigned int *channels,
                                            unsigned int *sampleRate,
                                            drwav_uint64 *totalSampleCount);
...
drwav_int16 *drwav_open_memory_and_read_s16(const void *data, size_t dataSize,
                                            ...
                                            unsigned int *sampleRate,
                                            drwav_uint64 *totalSampleCount);
float *drwav_open_memory_and_read_f32(const void *data, size_t dataSize,
                                      unsigned int *channels,
                                      unsigned int *sampleRate,
                                      drwav_uint64 *totalSampleCount);
drwav_int32 *drwav_open_memory_and_read_s32(const void *data, size_t dataSize,
                                            unsigned int *channels,
                                            unsigned int *sampleRate,
                                            drwav_uint64 *totalSampleCount);
#endif
...
static DRWAV_INLINE unsigned int
drwav__bytes_to_u32(const unsigned char *data) {
...
drwav_bool32 drwav_init_file_ex(drwav *pWav, const char *filename,
  ...
  return drwav_init_ex(pWav, drwav__on_read_stdio, drwav__on_seek_stdio,
                       onChunk, (void *)pFile, pChunkUserData, flags);
}
...
drwav_bool32 drwav_init_file_write__internal(drwav *pWav, const char *filename,
  ...
  return drwav_init_write__internal(pWav, pFormat, totalSampleCount,
                                    isSequential, drwav__on_write_stdio,
...
drwav *drwav_open_file_ex(const char *filename, drwav_chunk_proc onChunk,
  ...
  drwav *pWav = drwav_open_ex(drwav__on_read_stdio, drwav__on_seek_stdio,
                              onChunk, (void *)pFile, pChunkUserData, flags);
  if (pWav == NULL) {
    fclose(pFile);
    return NULL;
...
drwav *drwav_open_file_write__internal(const char *filename,
  ...
  drwav *pWav = drwav_open_write__internal(pFormat, totalSampleCount,
                                           isSequential, drwav__on_write_stdio,
                                           drwav__on_seek_stdio, (void *)pFile);
  if (pWav == NULL) {
    fclose(pFile);
...
drwav *drwav_open_file_write_sequential(const char *filename,
                                        const drwav_data_format *pFormat,
                                        drwav_uint64 totalSampleCount) {
  return drwav_open_file_write__internal(filename, pFormat, totalSampleCount,
                                         DRWAV_TRUE);
}
#endif // DR_WAV_NO_STDIO
...
drwav_bool32 drwav_init_memory_ex(drwav *pWav, const void *data,
  ...
  if (!drwav_init_ex(pWav, drwav__on_read_memory, drwav__on_seek_memory,
                     onChunk, (void *)&memoryStream, pChunkUserData, flags)) {
    return DRWAV_FALSE;
  }
...
drwav_bool32 drwav_init_memory_write__internal(drwav *pWav, void **ppData,
                                               size_t *pDataSize,
                                               const drwav_data_format *pFormat,
                                               drwav_uint64 totalSampleCount,
                                               drwav_bool32 isSequential) {
  ...
  *pDataSize = 0;
  ...
  if (!drwav_init_write__internal(
          pWav, pFormat, totalSampleCount, isSequential, drwav__on_write_memory,
...
drwav *drwav_open_memory_write__internal(void **ppData, size_t *pDataSize,
                                         const drwav_data_format *pFormat,
                                         drwav_uint64 totalSampleCount,
                                         drwav_bool32 isSequential) {
  if (ppData == NULL) {
  ...
  *pDataSize = 0;
  ...
  drwav *pWav = drwav_open_write__internal(
      pFormat, totalSampleCount, isSequential, drwav__on_write_memory,
      drwav__on_seek_memory_write, (void *)&memoryStreamWrite);
  if (pWav == NULL) {
    return NULL;
  }
...
drwav *drwav_open_memory_write(void **ppData, size_t *pDataSize,
                               const drwav_data_format *pFormat) {
  return drwav_open_memory_write__internal(ppData, pDataSize, pFormat, 0,
                                           DRWAV_FALSE);
}
...
drwav *drwav_open_memory_write_sequential(void **ppData, size_t *pDataSize,
                                          const drwav_data_format *pFormat,
                                          drwav_uint64 totalSampleCount) {
  return drwav_open_memory_write__internal(ppData, pDataSize, pFormat,
                                           totalSampleCount, DRWAV_TRUE);
}
...
drwav_bool32 drwav__on_seek(drwav_seek_proc onSeek, void *pUserData, int offset,
                            drwav_seek_origin origin, drwav_uint64 *pCursor) {
...
static drwav_uint32 drwav_get_bytes_per_sample(drwav *pWav) {
  // The number of bytes per sample is based on the bits per sample or the block
  // align. We prioritize floor(bitsPerSample/8), but if this is zero or the
  // bits per sample is not a multiple of 8 we need to fall back to the block
  // align.
...
static drwav_uint32 drwav_get_bytes_per_pcm_frame(drwav *pWav) {
  // The number of bytes per frame is based on the bits per sample or the block
  // align. We prioritize floor(bitsPerSample*channels/8), but if this is zero
  // or the bits per frame is not a multiple of 8 we need to fall back to the
  // block align.
...
drwav_bool32 drwav_init_ex(drwav *pWav, drwav_read_proc onRead,
  ...
  drwav_uint64 cursor = 0; // <-- Keeps track of the byte position so we can
  ...
  if (drwav__on_read(onRead, pReadSeekUserData, riff, sizeof(riff), &cursor) !=
      sizeof(riff)) {
    return DRWAV_FALSE;
  }
  ...
  if (drwav__fourcc_equal(riff, "RIFF")) {
    ...
    if (drwav__on_read(onRead, pReadSeekUserData, riff2, sizeof(riff2),
                       &cursor) != sizeof(riff2)) {
      return DRWAV_FALSE;
    }
  ...
  if (pWav->container == drwav_container_riff) {
    ...
    if (drwav__on_read(onRead, pReadSeekUserData, chunkSizeBytes,
    ...
    if (drwav__on_read(onRead, pReadSeekUserData, wave, sizeof(wave),
                       &cursor) != sizeof(wave)) {
      return DRWAV_FALSE;
    }
    ...
    if (drwav__on_read(onRead, pReadSeekUserData, chunkSize, sizeof(chunkSize),
  ...
  drwav_fmt fmt;
  if (!drwav__read_fmt(onRead, onSeek, pReadSeekUserData, pWav->container,
                       &cursor, &fmt)) {
  ...
  for (;;) {
    ...
    drwav_result result = drwav__read_chunk_header(
        onRead, pReadSeekUserData, pWav->container, &cursor, &header);
    ...
    // is that we would otherwise require a backwards seek which sequential mode
    // forbids.
    ...
    if (pWav->container == drwav_container_riff) {
      if (drwav__fourcc_equal(header.id.fourcc, "fact")) {
        ...
        if (drwav__on_read(onRead, pReadSeekUserData, &sampleCount, 4,
                           &cursor) != 4) {
          return DRWAV_FALSE;
        }
        chunkSize -= 4;
      ...
      if (drwav__guid_equal(header.id.guid, drwavGUID_W64_FACT)) {
        if (drwav__on_read(onRead, pReadSeekUserData, &sampleCountFromFactChunk,
                           8, &cursor) != 8) {
          return DRWAV_FALSE;
        }
        chunkSize -= 8;
    ...
    if (pWav->container == drwav_container_riff) {
      if (drwav__fourcc_equal(header.id.fourcc, "smpl")) {
        unsigned char
            smplHeaderData[36]; // 36 = size of the smpl header section, not
                                // including the loop data.
        if (chunkSize >= sizeof(smplHeaderData)) {
          drwav_uint64 bytesJustRead =
              drwav__on_read(onRead, pReadSeekUserData, smplHeaderData,
          ...
          if (bytesJustRead == sizeof(smplHeaderData)) {
            ...
            for (drwav_uint32 iLoop = 0;
                 iLoop < pWav->smpl.numSampleLoops &&
                 iLoop < drwav_countof(pWav->smpl.loops);
                 ++iLoop) {
              unsigned char smplLoopData[24]; // 24 = size of a loop section in
                                              // the smpl chunk.
              bytesJustRead =
                  drwav__on_read(onRead, pReadSeekUserData, smplLoopData,
      ...
      if (drwav__guid_equal(header.id.guid, drwavGUID_W64_SMPL)) {
        // This path will be hit when a W64 WAV file contains a smpl chunk. I
        // don't have a sample file to test this path, so a contribution is
        // welcome to add support for this.
      }
    }
  ...
  // chunk.
  if (!sequential) {
    if (!drwav__seek_from_start(onSeek, pWav->dataChunkDataPos,
                                pReadSeekUserData)) {
      return DRWAV_FALSE;
    }
  ...
  // however this results in the inclusion of extra silent samples at the end of
  // the last block. The correct way to know the total sample count is to
...
drwav_uint64 drwav_riff_chunk_size_w64(drwav_uint64 dataChunkSize) {
  return 80 + 24 + dataChunkSize; // +24 because W64 includes the size of the
...
drwav_init_write__internal(drwav *pWav, const drwav_data_format *pFormat,
  ...
  // not we are initializing in sequential mode or not. In sequential mode we
  // set this to its final values straight away since they can be calculated
  // from the total sample count. In non- sequential mode we initialize it all
  // to zero and fill it out in drwav_uninit() using a backwards seek.
...
drwav_bool32 drwav_init_write(drwav *pWav, const drwav_data_format *pFormat,
                              drwav_write_proc onWrite, drwav_seek_proc onSeek,
                              void *pUserData) {
  return drwav_init_write__internal(pWav, pFormat, 0, DRWAV_FALSE, onWrite,
                                    onSeek,
...
drwav_bool32 drwav_init_write_sequential(drwav *pWav,
                                         ...
                                         drwav_write_proc onWrite,
                                         void *pUserData) {
  return drwav_init_write__internal(pWav, pFormat, totalSampleCount, DRWAV_TRUE,
                                    onWrite, NULL,
...
void drwav_uninit(drwav *pWav) {
  ...
  // things:
  //   - Make sure the "data" chunk is aligned to 16-bits for RIFF containers,
  //   or 64 bits for W64 containers.
  //   - Set the size of the "data" chunk.
  if (pWav->onWrite != NULL) {
    // Validation for sequential mode.
    if (pWav->isSequentialWrite) {
      drwav_assert(pWav->dataChunkDataSize ==
                   pWav->dataChunkDataSizeTargetWrite);
    }
    ...
    // at initialization time. We only need to do this when using non-sequential
    // mode.
      ...
      if (pWav->container == drwav_container_riff) {
        ...
        if (pWav->onSeek(pWav->pUserData, 4, drwav_seek_origin_start)) {
          drwav_uint32 riffChunkSize =
              drwav_riff_chunk_size_riff(pWav->dataChunkDataSize);
        ...
        if (pWav->onSeek(pWav->pUserData, (int)pWav->dataChunkDataPos + 4,
          ...
          drwav_uint32 dataChunkSize =
              drwav_data_chunk_size_riff(pWav->dataChunkDataSize);
        ...
        if (pWav->onSeek(pWav->pUserData, 16, drwav_seek_origin_start)) {
          drwav_uint64 riffChunkSize =
              drwav_riff_chunk_size_w64(pWav->dataChunkDataSize);
        ...
        if (pWav->onSeek(pWav->pUserData, (int)pWav->dataChunkDataPos + 16,
          ...
          drwav_uint64 dataChunkSize =
              drwav_data_chunk_size_w64(pWav->dataChunkDataSize);
...
drwav *drwav_open_ex(drwav_read_proc onRead, drwav_seek_proc onSeek,
  ...
  if (!drwav_init_ex(pWav, onRead, onSeek, onChunk, pReadSeekUserData,
...
drwav *drwav_open_write__internal(const drwav_data_format *pFormat,
  ...
  if (!drwav_init_write__internal(pWav, pFormat, totalSampleCount, isSequential,
                                  onWrite, onSeek, pUserData)) {
    DRWAV_FREE(pWav);
    return NULL;
  }
...
drwav *drwav_open_write(const drwav_data_format *pFormat,
                        drwav_write_proc onWrite, drwav_seek_proc onSeek,
                        void *pUserData) {
  return drwav_open_write__internal(pFormat, 0, DRWAV_FALSE, onWrite, onSeek,
                                    pUserData);
}
...
drwav *drwav_open_write_sequential(const drwav_data_format *pFormat,
                                   drwav_uint64 totalSampleCount,
                                   drwav_write_proc onWrite, void *pUserData) {
  return drwav_open_write__internal(pFormat, totalSampleCount, DRWAV_TRUE,
                                    onWrite, NULL, pUserData);
}
...
drwav_uint64 drwav_read(drwav *pWav, drwav_uint64 samplesToRead,
  ...
  size_t bytesRead = drwav_read_raw(
      pWav, (size_t)(samplesToRead * bytesPerSample), pBufferOut);
...
drwav_uint64 drwav_read_pcm_frames(drwav *pWav, drwav_uint64 framesToRead,
  ...
  size_t bytesRead =
      drwav_read_raw(pWav, (size_t)(framesToRead * bytesPerFrame), pBufferOut);
...
drwav_uint64 drwav_write_pcm_frames(drwav *pWav, drwav_uint64 framesToWrite,
                                    const void *pData) {
  return drwav_write(pWav, framesToWrite * pWav->channels, pData) /
         pWav->channels;
}
...
drwav_uint64 drwav_read_s16__pcm(drwav *pWav, drwav_uint64 samplesToRead,
  ...
  while (samplesToRead > 0) {
    drwav_uint64 samplesRead = drwav_read(
        pWav, drwav_min(samplesToRead, sizeof(sampleData) / bytesPerSample),
        sampleData);
    if (samplesRead == 0) {
      break;
    }
...
drwav_uint64 drwav_read_pcm_frames_s16(drwav *pWav, drwav_uint64 framesToRead,
                                       drwav_int16 *pBufferOut) {
  return drwav_read_s16(pWav, framesToRead * pWav->channels, pBufferOut) /
         pWav->channels;
}
...
void drwav_s24_to_s16(drwav_int16 *pOut, const drwav_uint8 *pIn,
  ...
  for (size_t i = 0; i < sampleCount; ++i) {
    int x =
        ((int)(((unsigned int)(((const unsigned char *)pIn)[i * 3 + 0]) << 8) |
...
drwav_uint64 drwav_read_pcm_frames_f32(drwav *pWav, drwav_uint64 framesToRead,
                                       float *pBufferOut) {
  return drwav_read_f32(pWav, framesToRead * pWav->channels, pBufferOut) /
         pWav->channels;
}
...
drwav_uint64 drwav_read_pcm_frames_s32(drwav *pWav, drwav_uint64 framesToRead,
                                       drwav_int32 *pBufferOut) {
  return drwav_read_s32(pWav, framesToRead * pWav->channels, pBufferOut) /
         pWav->channels;
}
...
drwav_int16 *drwav_open_and_read_pcm_frames_s16(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  drwav_int16 *result = drwav_open_and_read_s16(
...
float *drwav_open_and_read_pcm_frames_f32(drwav_read_proc onRead,
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  float *result = drwav_open_and_read_f32(onRead, onSeek, pUserData, &channels,
...
drwav_int32 *drwav_open_and_read_pcm_frames_s32(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  drwav_int32 *result = drwav_open_and_read_s32(
      onRead, onSeek, pUserData, &channels, &sampleRate, &totalSampleCount);
  if (result == NULL) {
    return NULL;
  }
...
drwav_int16 *drwav_open_file_and_read_pcm_frames_s16(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  drwav_int16 *result = drwav_open_file_and_read_s16(
...
float *drwav_open_file_and_read_pcm_frames_f32(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  float *result = drwav_open_file_and_read_f32(filename, &channels, &sampleRate,
                                               &totalSampleCount);
  if (result == NULL) {
    return NULL;
  }
...
drwav_int32 *drwav_open_file_and_read_pcm_frames_s32(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  drwav_int32 *result = drwav_open_file_and_read_s32(
      filename, &channels, &sampleRate, &totalSampleCount);
  if (result == NULL) {
    return NULL;
  }
...
drwav_int16 *drwav_open_memory_and_read_pcm_frames_s16(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  drwav_int16 *result = drwav_open_memory_and_read_s16(
      data, dataSize, &channels, &sampleRate, &totalSampleCount);
  if (result == NULL) {
    return NULL;
  }
...
float *drwav_open_memory_and_read_pcm_frames_f32(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  float *result = drwav_open_memory_and_read_f32(
      data, dataSize, &channels, &sampleRate, &totalSampleCount);
  if (result == NULL) {
    return NULL;
  }
...
drwav_int32 *drwav_open_memory_and_read_pcm_frames_s32(
  ...
  unsigned int sampleRate;
  drwav_uint64 totalSampleCount;
  drwav_int32 *result = drwav_open_memory_and_read_s32(
      data, dataSize, &channels, &sampleRate, &totalSampleCount);
  if (result == NULL) {
    return NULL;
  }
...
//   samples. Old APIs have been deprecated and
