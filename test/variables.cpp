...
TEST(logic, less) {
    cactus::Graph g;
    auto x = cactus::Const(g.opName("x"), { { 1.0, 2.0 },{ 3.0, 4.0 } });
    auto y = cactus::Const(g.opName("y"), 3);
    auto z = cactus::lt(g.opName("z"), x, y);
    g.run(z);
    EXPECT_EQ(1, z.tensor().get<int>(0));
    EXPECT_EQ(1, z.tensor().get<int>(1));
    EXPECT_EQ(0, z.tensor().get<int>(2));
    EXPECT_EQ(0, z.tensor().get<int>(3));
    z = cactus::gt(g.opName("z"), x, y);
    g.run(z);
    EXPECT_EQ(0, z.tensor().get<int>(0));
    EXPECT_EQ(0, z.tensor().get<int>(1));
    EXPECT_EQ(0, z.tensor().get<int>(2));
    EXPECT_EQ(1, z.tensor().get<int>(3));
    z = cactus::ltoe(g.opName("z"), x, y);
    g.run(z);
    EXPECT_EQ(1, z.tensor().get<int>(0));
    EXPECT_EQ(1, z.tensor().get<int>(1));
    EXPECT_EQ(1, z.tensor().get<int>(2));
    EXPECT_EQ(0, z.tensor().get<int>(3));
    z = cactus::gtoe(g.opName("z"), x, y);
    g.run(z);
    EXPECT_EQ(0, z.tensor().get<int>(0));
    EXPECT_EQ(0, z.tensor().get<int>(1));
    EXPECT_EQ(1, z.tensor().get<int>(2));
    EXPECT_EQ(1, z.tensor().get<int>(3));
    z = cactus::eq(g.opName("z"), x, y);
    g.run(z);
    EXPECT_EQ(0, z.tensor().get<int>(0));
    EXPECT_EQ(0, z.tensor().get<int>(1));
    EXPECT_EQ(1, z.tensor().get<int>(2));
    EXPECT_EQ(0, z.tensor().get<int>(3));
    z = cactus::neq(g.opName("z"), x, y);
    g.run(z);
    EXPECT_EQ(1, z.tensor().get<int>(0));
    EXPECT_EQ(1, z.tensor().get<int>(1));
    EXPECT_EQ(0, z.tensor().get<int>(2));
...
TEST(flow, cond) {
    ...
    auto x = cactus::Const(g.opName("x"), { { 1, 2 },{ 3, 4 } });
    auto y = cactus::Const(g.opName("y"), { { 1, 2 },{ 3, 4 } });
    auto add = cactus::add(g, x, y);
    auto pro = cactus::product(g, x, y);
    auto z = cactus::cond(g, cactus::lt(g, x, cactus::Const(g,2)), add, pro);
    g.run(z);
    EXPECT_EQ(2, z.tensor().get<int>(0));
    EXPECT_EQ(4, z.tensor().get<int>(1));
    EXPECT_EQ(6, z.tensor().get<int>(2));
    EXPECT_EQ(8, z.tensor().get<int>(3));
}