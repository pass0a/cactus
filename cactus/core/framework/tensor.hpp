...
public:
  tensor() : storage_() {}
  tensor(const Storage &s) : storage_(s) {}
  tensor(const tensor &rhs) : storage_(rhs.storage_) {}
  explicit tensor(std::initializer_list<size_t> shape) : storage_(shape) {}
  explicit tensor(shape_type shape) : storage_(shape) {}
  explicit tensor(T val) : storage_(val) {}
